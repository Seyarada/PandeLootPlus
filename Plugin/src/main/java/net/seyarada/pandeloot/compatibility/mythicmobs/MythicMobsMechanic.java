...
public class MythicMobsMechanic implements ITargetedLocationSkill, ITargetedEntitySkill {
    ...
    public SkillResult castAtLocation(SkillMetadata skillMetadata, AbstractLocation abstractLocation) {
        String i = PlaceholderString.of(MythicLineConfigImpl.parseBlock(config.getLine())).get(skillMetadata);
        int j = i.indexOf("{")+1;
        int k = i.lastIndexOf("}");
    ...
    public SkillResult castAtEntity(SkillMetadata skillMetadata, AbstractEntity abstractEntity) {
        ...
        String i = PlaceholderString.of(MythicLineConfigImpl.parseBlock(config.getLine())).get(skillMetadata, abstractEntity);
        int j = i.indexOf("{")+1;
        int k = i.lastIndexOf("}");
        String dropString = i.substring(j, k);
